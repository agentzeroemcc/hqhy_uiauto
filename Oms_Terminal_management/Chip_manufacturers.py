...
class Chip(unittest.TestCase):
    ...
    def test_Newly_added(self):
        ...
        driver.find_element_by_xpath('//*[@id="collapseThree"]/div/ul/li/a').click()  #  点击芯片厂商列表
        ...
        homepage.create_chipvendor_form(name + '（测试）')  #  输入芯片厂商名称
        Select(driver.find_element_by_name('status')).select_by_value('2')   #  状态选择停用
        homepage.create_chipvendor_form_text(describe)  #  输入描述
        homepage.create_chipvendor_form_button()  #  点击保存
        try:
    ...
    def test_edit(self):
        ...
        homepage.edit_chipvendor_form()  #  点击保存
        try:
